    ...
    try {
      ...
      if (response.success) {
        ...
        if (authData.user) {
          ...
          const from = location.state?.from?.pathname || (authData.user.role === 'admin' ? "/dashboard" : "/survey-redirect");
          navigate(from);
        } else {
          const from = location.state?.from?.pathname || "/survey-redirect";
          navigate(from);
        }
      } else {
      ...
      if (isNetworkError) {
        ...
        if (offlineResult.success) {
          ...
          const from = location.state?.from?.pathname || (offlineResult.data.user.role === 'admin' ? "/dashboard" : "/survey-redirect");
          navigate(from);
          return;
        } else {
