...
export default function TakeSurvey() {
    const { id: surveyId } = useParams(); // Note the difference in param name if any, but App.js uses path="/survey/:id"
    ...
    const fetchParliaments = async () => {
        try {
            ...
            if (res.data.success) {
                ...
                const newOptions = res.data.data.map(p => ({ label: p, value: p }));
                setSurvey(prev => {
                    if (!prev) return prev;
    ...
    const fetchMunis = async (parl) => {
        try {
            ...
            if (res.data.success) {
                ...
                const newMuniOptions = res.data.data.map(m => ({ label: m, value: m }));
                setSurvey(prev => {
                    if (!prev) return prev;
                    const updatedQs = [...prev.questions];
    ...
    const fetchSurvey = async () => {
        try {
            ...
            // Fallback logic if needed, but for now we expect the API to work
        } finally {
            setLoading(false);
        }
    };
            ...
            <form onSubmit={handleSubmit} className="survey-form">
                {!isHeaderLocked && (
                    <div className="question-block" style={{ border: '1.5px dashed #6366f1', background: '#f8f9ff', borderRadius: '12px', padding: '1rem 2rem', marginBottom: '0.75rem' }}>
                        ...
                        <button type="button" className="btn btn-primary" style={{ width: '100%', padding: '0.75rem', fontSize: '14px' }} onClick={startBatch}>Confirm & Start Surveying</button>
                    </div>
                )}
